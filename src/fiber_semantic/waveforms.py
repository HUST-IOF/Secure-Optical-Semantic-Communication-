...
def load_features(path, variable="Data"):
    """Load a MATLAB acquisition; ordering follows the experiment's vector layout."""
    from scipy.io import loadmat
    data = loadmat(path)
    if variable not in data:
        raise ValueError(f"MATLAB variable {variable!r} is missing")
    signal = np.asarray(data[variable])
    if signal.size != 256 * 800 or not np.isfinite(signal).all():
        raise ValueError("Expected 256 symbols of 800 finite samples each")
    if signal.ndim > 2 or (signal.ndim == 2 and min(signal.shape) != 1):
        raise ValueError("Expected a MATLAB row or column vector")
    return process_full_vector(signal)[0]
