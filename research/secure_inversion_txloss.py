...
def materialize_receiver_prompts(
    ...
    for src_prompt in prompt_files:
        ...
        out_prompt = {
            'U': torch.from_numpy(U_rx).to(dtype=torch.uint8),
            'V': torch.from_numpy(V_rx).to(dtype=torch.uint8),
            'U_scale': prompt['U_scale'],
            'U_zero_point': prompt['U_zero_point'],
            'V_scale': prompt['V_scale'],
            'V_zero_point': prompt['V_zero_point'],
        }
...
def parse_freq_argument(freq_arg, all_freqs):
    """
    支持：
    ...
    if freq_arg.lower() == "all":
        return all_freqs

    # 逗号列表
    if "," in freq_arg:
        return [x.strip() for x in freq_arg.split(",")]

    # 范围
    if "-" in freq_arg:
        import re

        m = re.match(r"(\d+)\-(\d+)MHz", freq_arg)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--freq', type=str, default="40MHz",
                        help='Frequency interval, e.g., 40MHz. Use "all" for all available frequencies.')
