...
def generation(
    ...
    with precision_scope("cuda"):
        ...
        for prompt_pair in zip(prompts[::], prompts[1::]):
            ...
            # Public release supports portable CPU-saved prompt files.
            prompt_curr = torch.load(prompt_curr, map_location='cuda:0', weights_only=False)
            prompt_next = torch.load(prompt_next, map_location='cuda:0', weights_only=False)

            # low-rank factors
            U_curr, V_curr, U_next, V_next = prompt_curr['U'], prompt_curr['V'], prompt_next['U'], prompt_next['V']

            #print(U_curr, V_curr, U_next, V_next)
            ...
            if prev_frame is None:
                ...
                prev_frame = torch.load(os.path.join(prompt_dir, 'init.pth'), map_location='cuda:0', weights_only=False)
